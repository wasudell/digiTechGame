...
{   
    ...
    private void GetInput()
    {
        ...
        RaycastHit frontRaycast; // front raycast for directly in front of car, double velocity distance to prevent collisions earlier
        if (Physics.Raycast(transform.position, transform.TransformDirection(Vector3.forward), out frontRaycast, Mathf.Abs(velocity * 2))){
            frontHit = true;
            Debug.DrawRay(transform.position, transform.TransformDirection(Vector3.forward) * frontRaycast.distance, Color.red);
        }
        else {
            frontHit = false;
        }
        RaycastHit leftCornerRaycast; // left corner for if the front raycast can't see an obstacle, half velocity distance as little adjustment is needed to avoid here
        if (Physics.Raycast(transform.position, transform.TransformDirection(new Vector3(0.5f, 0, 1)), out leftCornerRaycast, Mathf.Abs(velocity / 2))){
            leftCornerDistance = leftCornerRaycast.distance;
            leftCornerHit = true;
            Debug.DrawRay(transform.position, transform.TransformDirection(new Vector3(0.5f, 0, 1)) * leftCornerRaycast.distance, Color.red);
        }
        else {
            leftCornerHit = false;
        }
        RaycastHit rightCornerRaycast; // same as left corner but now for the right corner
        if (Physics.Raycast(transform.position, transform.TransformDirection(new Vector3(-0.5f, 0, 1)), out rightCornerRaycast, Mathf.Abs(velocity / 2))){
    ...
    private void calculateAction()
    {
        ...
        if (frontHit == true && reversing != true){
            ...
            if (Physics.Raycast(transform.position, (transform.TransformDirection(new Vector3(-0.1f, 0, 1))), out leftRaycast, Mathf.Infinity)){
                leftDistance = leftRaycast.distance;
                Debug.DrawRay(transform.position, (transform.TransformDirection(new Vector3(-0.1f, 0, 1))) * leftRaycast.distance, Color.red);
            }
            RaycastHit rightRaycast; // same as left raycast but for right
            if (Physics.Raycast(transform.position, (transform.TransformDirection(new Vector3(0.1f, 0, 1))), out rightRaycast, Mathf.Infinity)){
                rightDistance = rightRaycast.distance;
                Debug.DrawRay(transform.position, (transform.TransformDirection(new Vector3(0.1f, 0, 1))) * rightRaycast.distance, Color.red);
            }
        ...
        if (avoiding == false && reversing == false){
            ...
            // turn right a lot as angle to van is greater than max steering angle, low speed
            if (relativeAngle >= 35 & relativeAngle <= 180){
                turnFactor = 1;
                vroomFactor = 0.3f;
            }
            // turn left a lot as angle to van is greater than max steering angle, low speed
            ...
            velocityNeededToBrake = Mathf.Abs(1 / (0.6f * turnFactor)) + 12;
    ...
    IEnumerator Braking(){
        // braking for 0.8 seconds if turn angle is too high
        isBraking = true;
